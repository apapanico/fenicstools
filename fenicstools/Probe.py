...
"""
This module contains functionality for efficiently probing a Function many
times.
"""
...
dolfin_folder = os.path.abspath(os.path.join(
    inspect.getfile(inspect.currentframe()), "../Probe"))
sources = ["Probe.cpp", "Probes.cpp",
           "StatisticsProbe.cpp", "StatisticsProbes.cpp"]
header_files = ['Probe.h', 'Probes.h', 'StatisticsProbe.h',
                'StatisticsProbes.h']
headers = [os.path.join(dolfin_folder, x) for x in header_files]
code = strip_essential_code(headers)
compiled_module = compile_extension_module(
    code=code, source_directory=os.path.abspath(dolfin_folder),

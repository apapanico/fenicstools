...
from dolfin import (compile_extension_module, assemble, TrialFunction,
                    TestFunction, dx, PETScMatrix, VectorFunctionSpace, dot)
...
def cg1_cr_interpolation_matrix(mesh, constrained_domain=None):
    ...
    CG1 = VectorFunctionSpace(
        mesh, 'CG', 1, constrained_domain=constrained_domain)
    CR = VectorFunctionSpace(
        mesh, 'CR', 1, constrained_domain=constrained_domain)

    # Get the matrix with approximate sparsity as the interpolation matrix
    u = TrialFunction(CG1)
    v = TestFunction(CR)
    I_mat = PETScMatrix()
    assemble(dot(u, v) * dx, tensor=I_mat)

    # Fill the interpolation matrix
    d = mesh.geometry().dim()
    compiled_i_module.compute_cg1_cr_interpolation_matrix(I_mat, d)
